...
(function () {
  ...
  const projects = {
    ...
    training: {
      ...
      summary: '从 Alcheme3.0 迁移多模态情绪识别能力，把服务话术训练升级为语音、文本、情绪和评分闭环的 AI 陪练系统。',
      role: '设计服务员场景、顾客角色、多模态情绪识别、SOP 规则检索、话术评分、训练档案和移动端 Demo 交互。',
      intro: '项目围绕头部餐饮品牌新人岗前服务训练，基于 Alcheme3.0 的情绪识别链路迁移到一线服务场景。新人通过语音或文本完成服务回应，系统识别顾客情绪、服务意图和风险等级，再结合 SOP 规则给出评分、纠偏建议和下一轮训练任务，帮助培训流程从一次性讲解变成可反复练习、可追踪复盘的陪练体验。',
      demo: '已有在线 Demo；展示位支持上传移动端操作录屏、服务场景截图和反馈页截图。',
      background: [
        '用户侧：餐饮新人需要高频练习真实顾客提问、投诉、催促和特殊需求，但门店人工陪练时间有限，且新人很难判断顾客情绪是否已经被安抚',
        '业务侧：一次性讲解难以保证话术稳定，培训负责人也缺少对话过程、情绪变化、薄弱场景和复练进度记录',
        '系统侧：小捞陪练不是单轮聊天 Demo，而是把 Alcheme3.0 的“视频/语音/文本情绪识别 → 结果融合 → 个性化反馈”迁移到服务训练',
        'AI 优势：语音识别记录新人回应，文本模型判断服务意图，情绪模型识别顾客状态，SOP 检索和规则评分共同约束反馈结果',
        '安全边界：系统只做服务训练和话术反馈，不替代真实门店管理；高风险投诉、食品安全和人身冲突场景需要提示升级给店长'
      ],
      goalMetrics: [
        '用户侧指标（练习完成率、重复练习率、情绪安抚得分）：新人能在移动端重复练习多个服务场景，并看到顾客情绪从不满到缓和的变化',
        '业务侧指标（人工带教时长、标准话术覆盖率、高风险场景升级率）：降低人工带教依赖，让标准话术、服务动作和升级规则更容易复用',
        '系统侧指标（ASR 成功率、情绪识别置信度、SOP 命中率、JSON 解析成功率）：语音转写、情绪识别、规则检索和结构化评分链路需要稳定运行',
        '评测侧指标（Rubric 一致性、低分归因准确率、复练提升率）：评分不只给总分，还要能解释扣分原因并验证下一轮是否提升'
      ],
      action: [
        '多模态输入：支持新人语音/文本回应，保留后续接入表情、语速、停顿和音色情绪特征的接口',
        '情绪融合：复用 Alcheme 的多模态思路，把语音情绪、文本情绪和顾客上下文合成顾客状态、冲突等级和置信度',
        'RAG 与规则：将服务 SOP、禁用话术、投诉处理流程和升级规则整理为知识库，先检索规则，再生成反馈，最后做合规校验',
        'Agent 分层：按“场景生成 Agent → 顾客模拟 Agent → 情绪识别 Agent → SOP 检索 Agent → Rubric 评分 Agent → Coach 反馈 Agent”组织训练流程',
        '训练状态：记录 trainee_profile、training_session、scenario_attempt、score_history 和 weakness_tags，用薄弱标签推荐下一轮训练',
        '兜底机制：ASR 失败时回退文本输入，低置信情绪判断提示人工确认，高风险场景不继续生成安抚话术而提示升级处理',
        '脱敏处理：页面文案弱化具体品牌，只保留餐饮服务话术陪练的可迁移方法'
      ],
      evidence: [
        '量化口径：完成练习次数、ASR 成功率、情绪安抚得分、SOP 命中率、反馈命中率、复练提升率和人工带教节省时间可作为验证指标',
        '过程证据：已有 GitHub 项目和在线 Demo，可展示移动端场景选择、对话练习、即时反馈页和训练链路',
        '过程证据：与 Alcheme3.0 共享“情绪识别 → 结果融合 → 个性化反馈”的能力底座，但服务场景增加 SOP 检索、规则校验和训练档案',
        '结果判断：案例证明的是把多模态情绪识别能力迁移到 B 端培训系统，而不是只做一个客服聊天窗口'
      ],
      screens: [
        {
